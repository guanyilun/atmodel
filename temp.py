...
def generic_temp(file_name, freq_range):
    ...
    freq_list, temp_list = bling.get_col(file_name, "Hz", 0, "K", 0, freq_range)
...
def radiance(site_file, freq_range):
    ...
    freq_list, radiance_list = bling.get_col(site_file, "Hz", 0, 0, "TOTAL RAD", freq_range)
...
def cmb(freq_range):
    ...
    freq_list, zero_list = bling.bling.generate_freq(freq_range)
...
def mirror(mirror_temp, constant, freq_range):
    ...
    if mirror_temp == -1 or constant == -1:
        freq, zero = bling.generate_freq(freq_range)
        return zero, freq
    
    # compute thermal noise for a set of frequencies
    freq_list, zero_list = bling.generate_freq(freq_range)
